...
INSERT INTO `config_info` VALUES (25, 'twelvet-server-gen-dev.yml', 'DEFAULT_GROUP', '# spring配置\nspring: \n  datasource:\n    dynamic:\n      hikari:\n        # 连接测试查询\n        connection-test-query: SELECT 1 FROM DUAL\n        # 连接最大存活时间.不等于0且小于30秒，会被重置为默认值30分钟.设置应该比mysql设置的超时时间短\n        max-lifetime: 540000\n        # 只有空闲连接数大于最大连接数且空闲时间超过该值，才会被释放\n        idle-timeout: 500000\n        # 最小空闲连接，默认值10，小于0或大于maximum-pool-size，都会重置为maximum-pool-size\n        minimum-idle: 10\n        # 最大连接数，小于等于0会被重置为默认值10；大于零小于1会被重置为minimum-idle的值\n        maximum-pool-size: 12\n        # 连接超时时间:毫秒，小于250毫秒，否则被重置为默认值30秒\n        connection-timeout: 60000\n      # 设置默认的数据源或者数据源组,默认值即为master\n      primary: master\n      datasource:\n        # 主库数据源\n        master:\n          driver-class-name: com.mysql.cj.jdbc.Driver\n          url: jdbc:mysql://twelvet-mysql:3306/twelvet?allowPublicKeyRetrieval=true&useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=false&serverTimezone=GMT%2B8\n          username: root\n          password: 123456\n        # 从库数据源\n        # slave:\n        # username:\n        # password:\n        # url:\n        # driver-class-name:\n      # 开启seata代理，开启后默认每个数据源都代理,分布式事务必须开启,否则关闭\n      seata: false\n  redis:\n    host: twelvet-redis\n    port: 6379\n    # 连接超时时间\n    timeout: 30s\n    lettuce:\n      pool:\n        # 连接池中的最小空闲连接\n        min-idle: 0\n        # 连接池中的最大空闲连接\n        max-idle: 8\n        # 连接池的最大数据库连接数\n        max-active: 8\n        # #连接池最大阻塞等待时间（使用负值表示没有限制）\n        max-wait: -1ms\n\n# 代码生成\ngen: \n  # 作者\n  author: TwelveT\n  # 默认生成包路径 system 需改成自己的模块名称 如 system monitor tool\n  packageName: com.twelvet.server.system\n  # 自动去除表前缀，默认是false\n  autoRemovePre: false\n  # 表前缀（生成类名不会包含表前缀，多个用逗号分隔）\n  tablePrefix: sys_\n\nswagger:\n  title: TwelveT Swagger API\n  version: 2.7.0\n  description: CRUD服务\n  license: Powered By TwelveT\n  licenseUrl: https://twelvet.cn\n  terms-of-service-url: https://twelvet.cn\n  contact:\n    name: TwelveT\n    email: 2471835953@qq.com\n    url: https://twelvet.cn\n  authorization:\n    name: password\n    token-url-list:\n      - http://${GATEWAY_HOST:localhost}:${GATEWAY_PORT:88}/auth/oauth/token\n# 开启Swagger增强模式\nknife4j:\n  enable: true', 'e4250a7021366c76d2200303b1afabc6', '2021-03-20 13:04:24', '2023-02-14 14:31:38', 'nacos', '127.0.0.1', '', 'eeb43899-8a88-4f5b-b0e0-d7c8fd09b86e', '代码生成器', 'null', 'null', 'yaml', 'null', '');

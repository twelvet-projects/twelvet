	...
	public boolean sendSysLoginLog(SysLoginInfo sysLoginInfo) {
		try {
			...
			Message<SysLoginInfo> message = MessageBuilder.withPayload(sysLoginInfo)
					.setHeader(MessageConst.PROPERTY_TAGS, "system-login").setHeader(MessageConst.PROPERTY_KEYS, "test")
					.build();

			// 发送事务消息（半消息操作）
			rocketMQTemplate.sendMessageInTransaction(MQGroupConstants.SYS_LOG_LOGIN, MQTopicConstants.QUEUE_LOG_LOGIN,
					message, null);
			return true;
		}
		catch (Exception e) {

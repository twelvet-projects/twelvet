...
public class EsCommonServiceImpl implements EsCommonService {
	...
	public EsCommonVO search(EsCommonDTO esCommonDTO) {
		...
		highlightBuilder.preTags("<span class='highlighted'>").postTags("</span>")
				// 高亮字段
				.field("title.like");
		...
		NativeSearchQuery nativeSearchQuery = nativeSearchQueryBuilder
				...
				.withSort(scoreSort).withQuery(boolQueryBuilder).withPageable(pageable)
				.withHighlightBuilder(highlightBuilder).build();
		SearchHits<EsCommon> searchHits = elasticsearchRestTemplate.search(nativeSearchQuery, EsCommon.class,
				INDEX_COMMON);
		...
		// 总数量
		long total = elasticsearchRestTemplate.count(nativeSearchQuery, EsCommon.class, INDEX_COMMON);
	...
	public void insert(EsCommon esCommon) {
		...
		if (!exists) {
			IndexQuery indexQuery = new IndexQueryBuilder().withObject(esCommon).withId(id).build();
	...
	private void buildFilter(EsCommonDTO esCommonDTO, BoolQueryBuilder boolQueryBuilder) {
		...
		// 分别搜索标题，内容
		boolQueryBuilder.should(QueryBuilders.termQuery("title.like", query).boost(100))

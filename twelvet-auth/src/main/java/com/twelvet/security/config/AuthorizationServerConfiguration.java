...
import com.twelvet.security.support.handler.TWTAuthenticationFailureEventHandler;
import com.twelvet.security.support.handler.TWTAuthenticationSuccessEventHandler;
import com.twelvet.security.support.password.OAuth2ResourceOwnerPasswordAuthenticationConverter;
	...
	public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http) throws Exception {
		...
		http.apply(authorizationServerConfigurer.tokenEndpoint((tokenEndpoint) -> {// 个性化认证授权端点
			tokenEndpoint.accessTokenRequestConverter(accessTokenRequestConverter()) // 注入自定义的授权认证Converter
					.accessTokenResponseHandler(new TWTAuthenticationSuccessEventHandler()) // 登录成功处理器
					.errorResponseHandler(new TWTAuthenticationFailureEventHandler());// 登录失败处理器
